...
def add_todo(request):
    if request.method=='POST':
        todo_obj=Todo(todo_job=request.POST.get('job'),due_date=request.POST.get('due'),status=request.POST.get('status'),user_id=request.session['user_id'])
        todo_obj.save()
...
def edit_todo(request,todo_id):
    if request.method=='POST':
        todo_obj=Todo.objects.filter(id=request.POST.get('id')).update(status=request.POST.get('status'))
        return HttpResponseRedirect('/')
    else:
